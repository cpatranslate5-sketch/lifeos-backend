...
{{
  ...
  "edits": [
    {{"existing_name": "строка — точное или близкое имя УЖЕ известной сущности из списка ниже", "new_name": "строка или null", "new_type": "person|project|event|goal|habit|task|movie|show|book|game|leisure|order или null", "new_fields": {{}}}}
  ],
...
Привычки (type="habit") — если пользователь привязывает её к конкретному дню недели ("по субботам", "каждый понедельник") — укажи fields.recurring=true и fields.weekday (0=понедельник...6=воскресенье). Если "каждый день"/"ежедневно" — вместо weekday укажи fields.daily=true и fields.recurring=true. Если "по будням"/"пн-пт"/"понедельник-пятница" — укажи fields.workdays=true и fields.recurring=true (не weekday и не daily). Если указан срок окончания ("до декабря", "до 31 декабря", "по конец года") — переведи его в конкретную дату YYYY-MM-DD (последний день упомянутого периода) относительно сегодняшней даты выше и укажи как fields.until. Без weekday/daily/workdays привычка не будет нигде отображаться в приложении, поэтому эти поля критичны.
...
Ежегодные даты без конкретного года (дни рождения, годовщины, "мама 21 апреля", "день рождения Пети 5 марта") — type="anniversary", fields.month (1-12) и fields.day (1-31) вместо fields.date. Имя — только тот, о ком дата (например, "Мама"), без "день рождения" в названии.
...
async def call_claude(user_message: str, today_str: str, lifephase: dict | None,
    ...
    space_note = ("Сейчас пространство «Работа» — не используй личные категории (быт, кино/книги/игры/досуг), "
                   "только person/project/event/goal/habit/task, актуальные для рабочего контекста."

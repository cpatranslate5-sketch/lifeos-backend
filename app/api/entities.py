...
def delete_entity(entity_id: str, db: Session = Depends(get_db)):
    ...
    if not e:
        raise HTTPException(404, "Entity not found")
    for fname in [e.attributes.get("cover_path"), e.attributes.get("background_path")] if e.attributes else []:
        if fname:
...
@router.post("/entities/{entity_id}/background", response_model=EntityOut)
async def upload_background(entity_id: str, file: UploadFile = File(...), db: Session = Depends(get_db)):
    e = db.get(Entity, entity_id)
    if not e:
        raise HTTPException(404, "Entity not found")
    ext = os.path.splitext(file.filename or "")[1].lower()
    ...
    old_bg = (e.attributes or {}).get("background_path")
    fname = f"{uuid.uuid4()}{ext}"
    with open(os.path.join(COVERS_DIR, fname), "wb") as out:
        out.write(content)
    if old_bg:
        old_path = os.path.join(COVERS_DIR, old_bg)
        if os.path.exists(old_path):
            os.remove(old_path)

    e.attributes = {**(e.attributes or {}), "background_path": fname}
    e.updated_at = now()
    db.commit()
    db.refresh(e)
    return EntityOut.model_validate(e)
...
def _bg_still_used_elsewhere(db: Session, fname: str, exclude_id: str) -> bool:
    for e in db.query(Entity).filter(Entity.is_active == True).all():  # noqa: E712
        if e.id != exclude_id and (e.attributes or {}).get("background_path") == fname:
            return True
    return False


@router.delete("/entities/{entity_id}/background", response_model=EntityOut)
def remove_background(entity_id: str, db: Session = Depends(get_db)):
    e = db.get(Entity, entity_id)
    if not e:
        raise HTTPException(404, "Entity not found")
    old_bg = (e.attributes or {}).get("background_path")
    attrs = dict(e.attributes or {})
    attrs.pop("background_path", None)
    e.attributes = attrs
    e.updated_at = now()
    db.commit()
    # Only remove the file itself if no other card is still using this same image.
    if old_bg and not _bg_still_used_elsewhere(db, old_bg, entity_id):
        old_path = os.path.join(COVERS_DIR, old_bg)
        if os.path.exists(old_path):
            os.remove(old_path)
    db.refresh(e)
...
class RemoveBackgroundScopeIn(BaseModel):
    scope: str  # "same_image" | "all"


@router.post("/entities/{entity_id}/background-remove-scope")
def remove_background_scope(entity_id: str, payload: RemoveBackgroundScopeIn, db: Session = Depends(get_db)):
    source = db.get(Entity, entity_id)
    if not source:
        raise HTTPException(404, "Entity not found")
    target_bg = (source.attributes or {}).get("background_path")

    siblings = (db.query(Entity)
                .filter(Entity.type == source.type, Entity.profile == source.profile,
                        Entity.space == source.space, Entity.is_active == True)  # noqa: E712
                .all())

    if payload.scope == "same_image":
        if not target_bg:
            raise HTTPException(400, "У этой карточки нет фона")
        affected = [e for e in siblings if (e.attributes or {}).get("background_path") == target_bg]
    else:  # "all"
        affected = [e for e in siblings if (e.attributes or {}).get("background_path")]

    removed_files = set()
    for e in affected:
        bg = (e.attributes or {}).get("background_path")
        attrs = dict(e.attributes or {})
        attrs.pop("background_path", None)
        e.attributes = attrs
        e.updated_at = now()
        if bg:
            removed_files.add(bg)
    db.commit()

    for fname in removed_files:
        if not _bg_still_used_elsewhere(db, fname, "__none__"):
            fpath = os.path.join(COVERS_DIR, fname)
            if os.path.exists(fpath):
                os.remove(fpath)
...
class BackgroundScopeIn(BaseModel):
    scope: str  # "all" | "pool"


@router.post("/entities/{entity_id}/background-scope")
def apply_background_scope(entity_id: str, payload: BackgroundScopeIn, db: Session = Depends(get_db)):
    source = db.get(Entity, entity_id)
    if not source:
        raise HTTPException(404, "Entity not found")
    new_bg = (source.attributes or {}).get("background_path")
    if not new_bg:
        raise HTTPException(400, "У этой карточки нет фона")
    ...
    if payload.scope == "all":
        for e in siblings:
            e.attributes = {**(e.attributes or {}), "background_path": new_bg}
            e.updated_at = now()
        db.commit()
    ...
    if payload.scope == "pool":
        ...
        for e in siblings:
            bg = (e.attributes or {}).get("background_path")
            if bg and bg not in pool:
                pool.append(bg)
        if new_bg not in pool:
            pool.append(new_bg)
        for i, e in enumerate(siblings):
            e.attributes = {**(e.attributes or {}), "background_path": pool[i % len(pool)]}
            e.updated_at = now()
        db.commit()
        return {"updated": len(siblings), "pool_size": len(pool)}

...
def list_entities(type: str | None = Query(default=None), db: Session = Depends(get_db)):
...
def create_entity(payload: EntityCreate, db: Session = Depends(get_db)):
    e = Entity(type=payload.type, name=payload.name, attributes=payload.attributes, is_active=True)
    db.add(e)
    db.flush()
